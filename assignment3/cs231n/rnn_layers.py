...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    sum = prev_h@Wh + x@Wx + b
    #next_h = prev_h@Wh
    #next_h += Wx@x
    #next_h += b
    next_h = np.tanh(sum)
    cache = x, Wx, Wh, prev_h, sum
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    x, Wx, Wh, prev_h, sum = cache
    dnext_h = (1 - np.square(np.tanh(sum))) * dnext_h
    dx = dnext_h @ Wx.T
    dWx = x.T @ dnext_h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    H = b.shape[0]
    new = np.zeros((T,N,D))
    h_temp = np.zeros((T,N,H))
    h = np.zeros((N,T,H))
    for t in range(T):
        new[t] = x[:, np.array([t]), :].reshape(N,D)
        if t==0:
            prev_h = h0
        else:
            prev_h = h_temp[t-1]
        h_temp[t], cache[t] = rnn_step_forward(new[t], prev_h, Wx, Wh, b)
    for n in range(N):
        h[n] = h_temp[:, np.array([n]), :].reshape(T,H)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    dWh = np.zeros((H,H))
    db = np.zeros(H)
    new_dh_loss = np.zeros((T,N,H))
    dx_temp = np.zeros((T,N,D))
    dprev_h = np.zeros((N,H))
    for t in range(T-1, -1, -1):
        new_dh_loss[t] = dh[:, np.array([t]), :].reshape(N,H)
        if t==T-1:
            # only the upstream gradient from the loss for last timestep
            dnext_h = new_dh_loss[t]
        else:
            # for other timesteps, add gradient coming from next timestep
            dnext_h = new_dh_loss[t] + dprev_h
        dx_temp[t], dprev_h, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dnext_h, cache[t])
        dWx += dWx_temp
        dWh += dWh_temp
        db += db_temp
    dh0 = dprev_h
    for n in range(N):
        dx[n] = dx_temp[:, np.array([n]), :].reshape(T,D)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
